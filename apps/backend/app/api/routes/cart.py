...
async def add_item_to_cart(
    ...
    user_cart = crud_cart.cart.get_or_create(session, owner_id=user_id)
    ...
    cart_item = crud_cart.cart_item.add_item(
        session, cart_id=user_cart.id, item_in=item_in
    )
    return cart_item
...
async def update_cart_item_quantity(
    ...
    """
    user_id = uuid.UUID(current_user.id)
    user_cart = crud_cart.cart.get_by_owner(session, owner_id=user_id)
    if not user_cart:
        raise HTTPException(status_code=404, detail="Cart not found")

    updated_item = crud_cart.cart_item.update_item_quantity(
...
async def remove_item_from_cart(
    ...
    removed_item = crud_cart.cart_item.remove_item(
