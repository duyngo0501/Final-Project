...
async def place_order(
    ...
    if user_id is None and not order_in.items: # Need items if guest
         raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cart items required for guest checkout")
    
    cart_items_data = await get_cart_items_for_user(user_id, db) if user_id else order_in.items
    ...
    for cart_item in cart_items_data:
        ...
        if not item_id or quantity is None or price is None:
             ...
             raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid item data in cart.")

        item_total = price * quantity
        total_amount += item_total
        
        order_items_to_create.append(OrderItemCreate(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save order.")
    ...
    email_sent = send_order_confirmation_email(to_email=order_in.customer_email, order_details=email_details)
    if not email_sent:
        # Log error but don't fail the order placement itself maybe?
        logger.error(f"Failed to send order confirmation email for order {order_id_str}")
    ...
    logger.info(f"Order {order_id_str} placed successfully for {order_in.customer_email}")
